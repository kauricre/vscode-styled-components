...
- **Faithful version measurement:** Probe A/B must load the css-languageservice copy resolved _from the TS plugin's directory_, not the extension's top-level copy, and record the resolved version + path in output.
...
| Resolve + report the css-languageservice version the plugin _actually_ uses | Task 1 (`resolve.js`), surfaced in Task 3/5 output                                          |
